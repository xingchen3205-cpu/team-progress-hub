...
export default function QuestionBankTab() {
  const { trainingQuestions, teamGroups } = Workspace.useWorkspaceContext();
  const { Download, HelpCircle, Search, SectionHeader, DemoResetNote, EmptyState, surfaceCardClassName } = Workspace;
  ...
  const categoryCounts = scopedQuestions.reduce<Record<string, number>>((result, question) => {
          ...
          <div className="mt-4 space-y-2">
            {teamOptions.map((team) => (
              <button
                ...
                key={team.id}
                onClick={() => setSelectedTeamGroupId(team.id)}
                type="button"
              >
          ...
          {Object.keys(categoryCounts).length > 0 ? (
            <div className="mt-4 flex flex-wrap gap-2">
              {Object.entries(categoryCounts).map(([category, count]) => (
                <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600" key={category}>
                  {category} {count}
                </span>
              ))}
            </div>
              ...
              scopedQuestions.map((question) => (
                <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm" key={question.id}>
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <span className="shrink-0 text-xs text-slate-400">录入：{question.createdByName}</span>
                  </div>
