...
describe("guest expert review links", () => {
  ...
  it("allows guest experts to see only their assigned projects ordered by roadshow order", () => {
    ...
    assert.match(route, /reviewDisplaySession\.findFirst/);
    assert.match(route, /status:\s*\{\s*not:\s*"closed"\s*\}/);
    assert.match(route, /tokenExpiresAt:\s*\{\s*gt:\s*now\s*\}/);
    assert.match(route, /orderBy:\s*\{\s*createdAt:\s*"desc"\s*\}/);
    assert.match(route, /projectOrders:\s*\{/);
    assert.match(route, /orderIndex/);
    assert.match(route, /orderIndexByPackageId\.get\(assignment\.reviewPackage\.id\) \?\? index/);
    assert.doesNotMatch(route, /reviewDisplayProjectOrder\.findMany/);
  ...
  it("keeps guest mobile scoring rigorous and advances to the next pending project", () => {
    ...
    assert.match(page, /parseScoreDraft/);
    assert.match(page, /scoreCents/);
    assert.match(page, /Math\.round\(score \* 100\)/);
    const scorePatternMatch = page.match(/const scoreDraftPattern = (\/[^\n]+\/);/);
    assert.ok(scorePatternMatch);
    const scoreDraftPattern = Function(`return ${scorePatternMatch[1]}`)() as RegExp;
    assert.equal(scoreDraftPattern.test("0.29"), true);
    assert.equal(scoreDraftPattern.test("0.58"), true);
    assert.equal(scoreDraftPattern.test("99.99"), true);
    assert.equal(scoreDraftPattern.test("100.00"), true);
    assert.equal(scoreDraftPattern.test("100.01"), false);
    assert.equal(scoreDraftPattern.test("1.234"), false);
    assert.doesNotMatch(page, /Number\.isInteger\(score \* 100\)/);
