...
describe("expert review v2 constraints", () => {
  ...
  it("keeps expert-facing review copy clean and uses a centered completion modal", () => {
    ...
    assert.match(shellSource, /南京铁道职业技术学院创新创业管理平台/);
  ...
  it("renders expert accounts in a standalone review portal shell", () => {
    ...
    assert.match(layoutSource, /title:\s*"南京铁道职业技术学院创新创业管理平台"/);
    assert.doesNotMatch(shellSource, /EXPERT REVIEW PORTAL|大学生创新大赛评审系统|大赛评审系统/);
    assert.doesNotMatch(
  ...
  it("allows isolated workspace accounts to load their own notification inbox during workspace bootstrap", () => {
    ...
    assert.match(getBlockMatch[0], /assertRole\(user\.role,\s*\["admin",\s*"school_admin",\s*"teacher",\s*"leader",\s*"member",\s*"expert",\s*"training_teacher"\]\)/);
